...
void WESocket::startServer()
{
    ...
    while(ros::ok())
    {
        ...
        ROS_INFO("Connected! Client IP:[%s:%d]! ClientSocketFd:[%d]", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port), clientSocket);
        printUserCMD();
        if(fork() == 0)
        {
            close(serverSocket);
...
void WESocket::connectServer()
{
    ...
    {
        ROS_INFO("Local IP: [%s:%d] ClientSocket:[%d]", inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port),clientSocket);
    }
    ...
    else ROS_INFO("Server IP Address [%s:%d]",serverIP.c_str(),port);
...
bool WESocket::sendData(void *data, int len, int socket)
{
    if(!connected)
        return false;
  send_lock.lock();
  int sendLen = send(socket, data, len, 0);
  send_lock.unlock();
...
void WESocket::envSubCallback(const std_msgs::String::ConstPtr& msge)
{
  ...
  {
    sendData((void*)&msg, sizeof(msg),clientSocket);
    if(sendTimes > 1)
        usleep(100000);
  }
}
...
void WESocket::onCmd(const char *cmd)
{
  ...
  //for(int i= 0 ; i < MAX_MSG_LEN; i++)
     // s[i]='';
  if(PEEK_CMD_SD(cmd, "se", 2, s, clientSocket))
  {
    EnvMsg msg;
    msg.type = ID_ENV_MSG;
    for(int i= 0 ; i < MAX_MSG_LEN; i++)
  ...
  else if(PEEK_CMD_DD(cmd, "s", 1, d, clientSocket))
  {
    StateMsg msg;
    msg.type = ID_STATE_MSG;
    msg.data = d;
  ...
  else if (PEEK_CMD_FFFD(cmd, "sp", 2, f1, f2, f3, clientSocket))
  {
    PoseMsg msg;
    msg.type = ID_POSE_MSG;
    msg.x = f1;
  ...
  else if (PEEK_CMD_SD(cmd, "st", 2, s, clientSocket))
  {
    TaskMsg msg;
    msg.type = ID_TASK_MSG;
    for(int i = 0; i < MAX_MSG_LEN; i++)
  ...
  else if (PEEK_CMD_SD(cmd, "sq", 2, s, clientSocket))
  {
    ActseqMsg msg;
    msg.type = ID_ACTSEQ_MSG;
   for(int i = 0; i < MAX_MSG_LEN; i++)
