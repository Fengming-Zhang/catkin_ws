...
//----add begin----
unsigned int CanMotor::getTorquePDOBaseID()
{
    return torquePDOBaseID;
}
...
void CanMotor::setTorquePDO()
{
  torquepdoId = torquePDOBaseID|ID;
  if(can->registePDO(torquepdoId, std::make_pair(&torque, &lock)))
  {
    can->Xmit(ID, torquePDOIndex, 1, (uint32)(0x80000000 | torquepdoId));
    can->Xmit(ID, torquePDOIndex, 2, (BYTE)PDOPeriod);
    can->Xmit(ID, torquePDOMapIndex, 0, (BYTE)0);
    can->Xmit(ID, torquePDOMapIndex, 1, (uint32)(0x60770010));
    can->Xmit(ID, torquePDOMapIndex, 0, (BYTE)1);
    can->Xmit(ID, torquePDOIndex, 1, (uint32)(torquepdoId));
//    ROS_INFO("set torque PDO success -----------------)");
  } else {
//      ROS_INFO("error set torque PDO");
  }
}

void CanMotor::stopTorquePDO()
{
   can->Xmit(ID, torquePDOIndex, 1, (uint32)(0x80000000 | torquepdoId));
}
//----add end--------
...
void CanMotor::SetTargetVel(int32 val)
{
  ...
  ROS_WARN("speed is %i",val * lastProfile.coefficient);
