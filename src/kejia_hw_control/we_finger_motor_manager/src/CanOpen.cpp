...
{
  if((id & 0xFFFFFF80) != PosePDOBaseID && (id & 0xFFFFFF80) != torquePDOBaseID)
  {
...
//          {
//              ROS_INFO("0x%08X - angle: %.2f\r",dataBuffer[i].ID, val);
//             //fflush(stdout);
//          }
//          if(dataBuffer[i].ID == (PosePDOBaseID |13))
//          {
//              ROS_INFO("0x%08X - angle: %.2f\r",dataBuffer[i].ID, val);
//             //fflush(stdout);
//          }
        ...
        else if((dataBuffer[i].ID & 0xFFFFFF80) == torquePDOBaseID) {
            val = (bytes_to_int16(dataBuffer[i].Data));
//            ROS_INFO("%f------------------", val);
            valMap[dataBuffer[i].ID].second->lock();
            *(valMap[dataBuffer[i].ID].first) = val;
            valMap[dataBuffer[i].ID].second->unlock();
...
//---add begin----------
void CanOpen::setTorquePDOBaseID(uint32 id)
{
  torquePDOBaseID = id;
}
//---add end------------
