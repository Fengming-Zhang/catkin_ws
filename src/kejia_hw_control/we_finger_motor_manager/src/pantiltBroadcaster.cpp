...
static const std::string g_odomFrame = "/odom";
...
void PantiltBroadcaster::updateParams()
{
    // update param
    string key;
    m_privNh.getParamCached("/treat_tf", treat_tf);
    if (m_privNh.ok() && m_privNh.searchParam("PanTilt", key))
    {
        ...
        m_privNh.getParamCached(key + "/kinect2_offx", kinect2_offx);
        m_privNh.getParamCached(key + "/kinect2_offy", kinect2_offy);
        m_privNh.getParamCached(key + "/kinect2_offz", kinect2_offz);

        m_privNh.getParamCached(key + "/kinect2_roll", kinect2_roll);
        m_privNh.getParamCached(key + "/kinect2_pitch", kinect2_pitch);
        m_privNh.getParamCached(key + "/kinect2_yaw", kinect2_yaw);
...
void PantiltBroadcaster::updateTransform(double panAng, double tiltAng, double elevHei, const ros::Time &time)
{
    ...
    Vector3D kinect2(kinect2_offx, kinect2_offy, kinect2_offz);
...
//    supportPose.translate(supporter);
    ...
    Pose3D kinect2Pose = kinectPose;
    kinect2Pose.translate(kinect2).rotateX(kinect2_roll).rotateZ(kinect2_yaw).rotateY(kinect2_pitch);
...
//            .rotateX(kinect_roll).rotateZ(kinect_yaw).rotateY(kinect_pitch);
    ...
    tf::Transform kinect2Trans;
    kinect2Trans.setOrigin(tf::Vector3(kinect2Pose.translation.x, kinect2Pose.translation.y,
                                       kinect2Pose.translation.z));
    RotationMatrix &k2m = kinect2Pose.rotation;
    kinect2Trans.setBasis(tf::Matrix3x3(k2m.c[0].x, k2m.c[1].x, k2m.c[2].x,
                                        k2m.c[0].y, k2m.c[1].y, k2m.c[2].y,
                                        k2m.c[0].z, k2m.c[1].z, k2m.c[2].z));
    m_tf->sendTransform(tf::StampedTransform(kinect2Trans, time, kinect_frame, "kinect2_link"));
    // camera_1394
    ...
    if (treat_tf)
    {
        tf::Transform mapTrans;
        mapTrans.setOrigin(tf::Vector3(0,0,0));
        mapTrans.setBasis(tf::Matrix3x3(1,0,0,0,1,0,0,0,1));
        //ROS_INFO("treating...");
        m_tf->sendTransform(tf::StampedTransform(mapTrans, time, "map",g_odomFrame));
    }
}
